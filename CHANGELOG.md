...
- **Link page: no "Upgrade to C Max" for a month already billed as C Max.** When the coming cycle is billed as another package by hand ("Package for this period", v27), the subscriber's /c/<token> page shows that package and its price for the cycle, says "Already on C Max for this cycle" where the Upgrade button was, and offers Continue and Discontinue only. A plan whose own package is C Max was already offered no upgrade.
- **Summary chart after refunds.** The Collected bars on the Summary's 12-month chart, and on the Analytics "Collected vs costs by month" chart, are now collected minus refunds for each month (by refund date), the same figure the Refunds tile and Net use. The tooltip shows the split ("collected Rs 63,040 (Rs 70,240 less Rs 7,200 refunds)"). The Analytics month table keeps its separate Collected and Refunds columns.
- **Refunding a one-time item lowers its total.** A refund on a one-time item undoes that much of the sale: the item reads "Rs 7,000 (Rs 10,000 less Rs 3,000 refunded)", what was kept counts against that lower total, and a fully paid item stays Paid with nothing on the balance due. Before, the refund put the refunded amount back on the balance as if the buyer had to pay it again. If a refund is larger than what was paid, the difference is owed. The refund form's preview says what the total becomes. Monthly plans are unchanged: a refund there still makes the month owed again.
- **Record payment on every unpaid or upcoming period.** Each red (unpaid) or grey (upcoming) period box has its own Record payment button. The box that needs money next asks as before ("Record Rs 4,200 for 24 Sep – 23 Oct 2026?"). A later box asks for one payment per period through it, because credit fills periods in order and cannot skip a month: "Record 2 payments totalling Rs 8,400 for Sep and Oct?", big Yes / Different amount / Not now; Yes records them one per period, each noted "for 24 Sep – 23 Oct 2026 · 1 of 2, paid up to Nov 23, 2026". A plan with nothing owed (ended, or paid ahead) opens the same box from its Record payment button ("Nothing is owed on this plan · it ended Aug 23, 2026.") with the short amount / currency / date / note form. The old six-field form now serves only Edit in the payment history.
- **Paid up to… asks the same way.** Pick the cycle (or a date) and it says "Record 3 payments totalling Rs 12,600 for Aug, Sep and Oct?" with a big Yes and Not now. Different amount opens the total, date and note fields: a total that matches is split one payment per period; a different total is one payment noted "paid up to …" and fills periods in order.
- Long status pills ("Overdue for Aug – Sep") wrap on phones instead of widening the Status column. Server VERSION and the badge are 30.
