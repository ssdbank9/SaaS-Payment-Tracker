...
def record_confirmation(token, user_id, cycle_start, answer, note, ip, user_agent):
    at = now_iso()
    with tx() as con:
        con.execute(
            "INSERT INTO confirmations (token, user_id, cycle_start, answer, note, ip, user_agent, at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (token, user_id, cycle_start, answer, note[:500], ip[:64], user_agent[:300], at),
        )
        _bump(con)
    return at
...
def latest_confirmation(token, cycle_start):
    ...
    try:
        row = con.execute(
            "SELECT token, user_id, cycle_start, answer, note, at FROM confirmations WHERE token = ? AND cycle_start = ? ORDER BY id DESC LIMIT 1",
            (token, cycle_start),
        ).fetchone()
...
def confirmations_for_cycle(cycle_start):
    """Latest answer per token for a cycle."""
    con = connect()
    try:
        rows = con.execute(
            "SELECT c.token, c.user_id, c.cycle_start, c.answer, c.note, c.at FROM confirmations c "
            "JOIN (SELECT token, MAX(id) AS mid FROM confirmations WHERE cycle_start = ? GROUP BY token) m ON m.mid = c.id ORDER BY c.at",
            (cycle_start,),
        ).fetchall()
